...
【本版本已切换为 preprocess_reref.py 数据处理】
- 支持单点重参考/导联差分：X' = X - X_ref
- 可选 drop_ref：删除参考通道（否则参考通道会变为全 0）

依赖：
  - preprocess_reref.py  (提供 get_data，内置 rereference)
  - LoadData.py          (preprocess_reref 会 import 它)
  ...
  你的工程结构常见为：
    Only_Encoder/dataLoad/
      ...
      └─ BCICIV_2a/
          ├─ s1/A01T.mat ...
          └─ ...
  本脚本会自动定位 BCICIV_2a 根目录（优先 ./BCICIV_2a）。
"""
...
# -----------------------------------------------------------------------------
# Import your preprocessing utilities (with re-referencing)
# -----------------------------------------------------------------------------
try:
    # 你现在的目录结构通常是同级文件
    ...
    try:
        ...
        raise ImportError(
            "无法 import preprocess_reref.get_data。请确认：\n"
            "1) preprocess_reref.py 与本脚本在同一目录；或\n"
            "2) dataLoad/ 作为包可被 import（含 __init__.py）\n"
        ) from e
...
# -----------------------------------------------------------------------------
# Path resolving (避免 dataLoad/dataLoad 重复路径)
# -----------------------------------------------------------------------------
def resolve_bcic2a_root(user_data_dir: Optional[str]) -> str:
    ...
    if user_data_dir is not None and str(user_data_dir).strip() != "":
        ...
        # if user points to parent that contains BCICIV_2a/
...
# CLI
# -----------------------------------------------------------------------------
def _parse_ref_channel(s: str) -> Union[str, int]:
    """
    支持两种输入：
      - 'Cz' / 'FCz' 等通道名（字符串）
      - '9' 这类纯数字，视为通道索引 int（0-based）
    """
    if s is None:
        return "Cz"
    ss = str(s).strip()
...
def _safe_tag(x: Union[str, int]) -> str:
    s = str(x)
    s = s.replace(" ", "")
    s = s.replace("/", "_")
    s = s.replace("\\", "_")
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=float,
        default=4.0,
        help="进一步裁剪长度(秒)。preprocess_reref.get_data 已默认裁剪 2s~6s(4s)，一般保持 4.0。",
    )
    ...
    p.add_argument(
        ...
        default=1,
        choices=[0, 1],
        help="是否启用单点重参考/导联差分：X' = X - X_ref（1=启用，0=关闭）。默认启用。",
    )
    p.add_argument(
        "--ref_channel",
        type=str,
        default="Cz",
        help="参考通道：可用通道名(如 Cz) 或 0-based 索引(如 9)。默认 Cz。",
    )
    p.add_argument(
        "--drop_ref",
        type=int,
        default=0,
        choices=[0, 1],
        help="是否删除参考通道（1=删除；0=保留但该通道会变为全0）。默认不删除。",
    )
...
def main() -> None:
    ...
    # re-reference config
    use_reref = bool(args.reref)
    drop_ref = bool(args.drop_ref)
    ref_channel = _parse_ref_channel(args.ref_channel)
    ...
    print("-" * 80)
    print(f"[REREF] use_reref={use_reref} | ref_channel={ref_channel} | drop_ref={drop_ref}")
    print("=" * 80)

    # 1) load data (official protocol): preprocess_reref.get_data will append 's{subject}/'
    ...
    if target_len > X_train.shape[-1]:
        raise ValueError(
            f"--input_seconds 对应长度 {target_len} > preprocess_reref.get_data 输出长度 {X_train.shape[-1]}。\n"
    ...
    # 5) outputs dir (带上 reref tag，避免覆盖对照实验)
    exp_tag = f"sub{args.subject}"
    if use_reref:
        exp_tag += f"_reref-{_safe_tag(ref_channel)}_drop{int(drop_ref)}"

    out_dir = os.path.join(args.out_dir, exp_tag)
    ...
    # 9) attention averaging evidence on TEST
